...
void addRpgObject(std::shared_ptr<UnityBridge>& unity_bridge_ptr){
  // Initialize gates
...
int main(int argc, char *argv[]) {
  ...
  ros::NodeHandle nh("");
  ros::NodeHandle pnh("~");
  ros::Rate(50.0); //tw: wait a little bit
  ...
  QuadState quad_state; //quadrotor state
  quad_state.setZero(); 
  quad_ptr->reset(quad_state);
  Vector<3> quad_size(1.0, 1.0, 1.0); // define quadsize scale (for unity visualization only)
  ...
  addRpgObject(unity_bridge_ptr);

  unity_ready = unity_bridge_ptr->connectUnity(UnityScene::INDUSTRIAL);
  unsigned frame_id = 0;
  ...
  while(ros::ok() && unity_ready){
    ...
    /* 
      ...
      see rpg_quadrotor_control/trajectory_planning/polynomial_trajectories/
          include/polynomial_trajectories/polynomial_trajectories_common.h
